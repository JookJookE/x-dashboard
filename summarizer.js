...
4. 🚨가장 중요한 서식 규칙🚨: 모든 문장(줄)이 끝날 때마다 반드시 **엔터(줄바꿈)를 두 번** 쳐서 문장과 문장 사이에 항상 '빈 줄(공백 줄)'이 들어가게 하세요. 블라인드 폭로글처럼 띄엄띄엄 여백이 많고 읽기 편하게 작성해야 합니다.
5. '📌', '📝', '🗣️', '💬', '댓글(타래)용 링크' 같은 어색한 헤더, 서식, 외부 링크는 붙이지 마세요.
6. 마지막 줄에는 #블라인드 #직장인썰 해시태그 2개를 자연스럽게 넣으세요.
7. 오직 완성된 한글 트윗 문구만 출력하세요. (설명, 큰따옴표 금지)
`;
    } else if (mode === 'pann') {
      promptText = `
당신은 X(트위터)에서 연애, 결혼, 직장, 인간관계 사연으로 엄청난 리트윗(RT)과 공감을 이끌어내는 트위터 장인이자 인기 계정입니다.
제공된 사연 제목과 내용을 바탕으로, 트위터 피드에서 생생하게 공감을 얻는 '네이트판/사연 요약 트윗'을 한글로 작성하세요.

[사연 제목]: ${article.title}
[사연 내용]: ${article.contentSnippet || article.excerpt}

⚠️ [버전 6 - ⚖️ 네이트판 / 사연 작성 규칙]:
1. 🚨[팩트 엄수 중시]🚨: 본문 내용에 존재하지 않는 무분별한 뇌피셜(지어낸 남편, 아내, 시댁 등 등장인물이나 안 나온 사실)을 절대로 거짓으로 덧붙이지 마세요! 오직 본문에 실제 나오는 내용만 팩트 기반으로 요약하세요.
2. [첫 줄: 글쓴이/사연의 핵심 주장 요약 1문장]
3. [둘째 줄: 본문에서 밝힌 구체적인 이유 1문장]
4. [셋째 줄: 네티즌들의 반응이나 소통을 유도하는 질문 + 이모지]
5. 💬 [말투 & 어조 (버전 3 생생한 썰 감성)]: 말투는 트위터 피드 특유의 자연스럽고 생생한 썰/소통 톤을 그대로 유지하세요. 문장 끝 어조와 이모지는 사연 분위기(놀라움 😳, 유쾌함 ㅋㅋㅋ, 경악 😱, 호기심 👀, 불꽃 🔥 등)에 맞게 그때그때 다채롭게 연출하세요. (절대 매번 똑같이 'ㅠ'로 끝나지 않도록 주의!)
6. 🚨[서식 규칙]🚨: 모든 문장(줄)이 끝날 때마다 반드시 **엔터(줄바꿈)를 두 번** 쳐서 문장과 문장 사이에 항상 '빈 줄(공백 줄)'이 들어가게 하세요. 띄엄띄엄 여백이 많고 읽기 편하게 작성해야 합니다.
7. '📌', '📝', '🗣️', '💬', '댓글(타래)용 링크' 같은 어색한 헤더나 서식은 절대 붙이지 마세요.
