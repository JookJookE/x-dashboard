...
`;
      ...
      promptText = `
당신은 X(트위터)에서 연애, 결혼, 직장, 인간관계 사연으로 엄청난 리트윗(RT)과 갑론을박을 이끌어내는 커뮤니티 전문 큐레이터이자 트위터 장인입니다.
제공된 사연 제목과 내용을 바탕으로, 트위터 유저들의 공감과 참견을 유발하는 '사연 요약 트윗'을 한글로 작성하세요.

[사연 제목]: ${article.title}
[사연 내용]: ${article.contentSnippet || article.excerpt}

⚠️ [버전 6 - ⚖️ 네이트판 / 사연 작성 규칙]:
1. 🚨[팩트 엄수 중시]🚨: 본문 내용에 존재하지 않는 무분별한 뇌피셜(지어낸 남편, 아내, 시댁 등 등장인물이나 안 나온 사실)을 절대로 거짓으로 덧붙이지 마세요! 오직 본문에 실제 나오는 내용(글쓴이의 주장, 이유 등)만 팩트 기반으로 요약하세요.
2. [첫 줄: 글쓴이/사연의 핵심 주장 요약 1문장]
3. [둘째 줄: 본문에서 밝힌 구체적인 이유 1문장]
4. [셋째 줄: 네티즌들의 반응이나 소통을 유도하는 질문 + 이모지 1개]
5. 🚨[서식 규칙]🚨: 모든 문장(줄)이 끝날 때마다 반드시 **엔터(줄바꿈)를 두 번** 쳐서 문장과 문장 사이에 항상 '빈 줄(공백 줄)'이 들어가게 하세요. 여백이 많고 띄엄띄엄 읽기 편하게 작성해야 합니다.
6. '📌', '📝', '🗣️', '💬', '댓글(타래)용 링크' 같은 어색한 헤더나 서식은 절대 붙이지 마세요.
7. 마지막 줄에는 #네이트판 #네티즌갑론을박 해시태그 2개를 자연스럽게 넣으세요.
8. 오직 완성된 한글 트윗 문구만 출력하세요. (설명, 큰따옴표 금지)
`;
    } else if (mode === 'story') {
      promptText = `
