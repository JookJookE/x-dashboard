...
`;
      ...
      promptText = `
당신은 X(트위터)에서 연애, 결혼, 직장, 인간관계 갈등(AITA) 썰로 엄청난 리트윗(RT)과 갑론을박을 이끌어내는 커뮤니티 전문 큐레이터이자 트위터 장인입니다.
네이트판, 온라인 커뮤니티의 사연 기사를 바탕으로, 트위터 유저들의 과몰입과 참견을 유발하는 '도파민 자극형 사연 요약 트윗'을 한글로 작성하세요.

[사연 제목]: ${article.title}
[사연 내용]: ${article.contentSnippet || article.excerpt}

⚠️ [버전 6 - ⚖️ 네이트판 / 사연 갈등 유발 썰 작성 규칙]:
1. [첫 줄: 사연의 핵심 갈등을 요약하는 1문장 (예: 상견례에서 예비 시어머니가 이래서 파혼했다는데;;)]
2. [둘째 줄: 가장 어이없거나 분노를 유발하는 디테일 1문장]
3. [셋째 줄: 네티즌들의 참견과 갑론을박을 유도하는 질문 (예: 다들 이거 누가 잘못한 거 같아? 나만 이해 안 됨? 😳)]
4. 🚨가장 중요한 서식 규칙🚨: 모든 문장(줄)이 끝날 때마다 반드시 **엔터(줄바꿈)를 두 번** 쳐서 문장과 문장 사이에 항상 '빈 줄(공백 줄)'이 들어가게 하세요. 여백이 많고 띄엄띄엄 읽기 편하게 작성해야 합니다.
5. '📌', '📝', '🗣️', '💬', '댓글(타래)용 링크' 같은 어색한 헤더나 서식은 절대 붙이지 마세요.
6. 마지막 줄에는 #네이트판 #네티즌갑론을박 해시태그 2개를 자연스럽게 넣으세요.
7. 오직 완성된 한글 트윗 문구만 출력하세요. (설명, 큰따옴표 금지)
`;
    } else if (mode === 'story') {
      promptText = `
