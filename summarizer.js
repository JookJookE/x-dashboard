...
1. 🚨[어색한 라벨 절대 사용 금지]🚨: "📌 [아티클 제목]:", "📝 [아티클 본문]:", "💬 [댓글(타래)용 링크 텍스트]:" 같은 설명 라벨은 절대로 붙이지 마세요!
2. [첫 줄 - 도파민 훅 제목 1줄]: 독자가 손을 멈출 강렬한 호기심/도파민 자극형 제목 1줄만 작성하세요. (예: 반도체 호황 끝나간다고? 한국 경제 3%대 폭발 시나리오... 개미들만 모르는 진짜 대폭등 방아쇠가 당겨졌다!)
3. [빈 줄 한 칸(엔터 두 번)]
4. [본문 - 1인칭 분석 리포트 2~3문장]: [기사 팩트 1문장]. 아직 사람들은 [표면적 수치]만 보지만, 저는 진짜 핵심은 [실물 유동성/구조적 변화]에 있다고 생각합니다. [수혜 동력]이 [자산 시장]의 하방을 지지하는 결정적 동력이기 때문입니다. 이 거대한 파이프라인은 포트폴리오에 어떤 파급력으로 돌아올까요?
5. [빈 줄 한 칸(엔터 두 번)]
6. [마지막 줄 - 원본 링크]: 👇 본문 원본 및 3분 심층 리포트 풀버전 보기: ${article.link || ''}
7. 오직 완성된 깨끗한 최종 결과만 출력하세요. (생각 과정, 큰따옴표, 어색한 라벨 금지)
`;
    } else {
      promptText = `
