...
- 금지 단어/호칭: "당신", "귀하", "엑친", "엑친들", "액친", "액친들", "트친", "트친들", "요약하자면", "결론적으로", "놀랍게도", "과연", "안녕하세요", "정리하자면", "살펴보겠습니다", "알아보겠습니다"
- 번역투 및 딱딱한 문어체 어투 금지 ("~하는가?", "~인가?", "~바란다", "~것인가?" 등 어색한 직역체 말투 절대 금지)
...
async function generateTwitterSmallTalk() {
  ...
  const topics = [
    { type: 'food', prompt: '점심이나 저녁, 야식 메뉴, 커피/디저트 취향에 대해 사람들에게 툭 던지며 추천이나 투표를 유도하는 침샘 자극/결정장애 스몰톡 트윗' },
    { type: 'workplace', prompt: '직장 생활, 월요병/퇴근, 회의/업무 번아웃, 사회생활 현실에 대해 직장인 다들 폭풍 공감하고 리트윗할 1~2줄 사이다/유머 트윗' },
    { type: 'balance_game', prompt: '다들 사이에서 열띤 토론과 인용(Quote)이 폭발할 만한 재미있고 사소한 밸런스 게임 질문 (예: 평생 탄산 끊기 vs 평생 라면 끊기 등)' },
    { type: 'curiosity', prompt: '다들 요즘 뭐하고 사는지, 최근 꽂힌 음악/드라마/취미/소비 꿀템이 뭔지 사람들에게 편하게 물어보는 다정한 소통 트윗' },
  ...
  if (apiKey) {
    const promptText = `
당신은 X(트위터)에서 수만 명의 팔로워(다들)와 편하게 반말이나 친근한 어투로 소통하는 인기 트위터리안입니다.
지금 타임라인에 뜬금없이 툭 던져서 다들의 답글(멘션)과 인용(Quote)이 폭발할 '소통/스몰톡 트윗'을 한글로 작성하세요.

[이번 트윗 테마]: ${selectedTopic.prompt}

⚠️ [트위터 감성 소통 작성 규칙]:
1. 🚨[교과서/뉴스/존댓말 앵커 말투 절대 금지]🚨
   - 다들과 친구처럼 수다 떠는 톤 ("야 다들아", "다들 ~하냐?", "솔직히 ~아님?", "진짜 궁금해서 묻는데")
...
`;
  ...
  const fallbackList = [
    '솔직히 평생 치킨 끊기 vs 평생 라면 끊기 뭐 고름? 난 전자 절대 못함..',
...
- 3줄: 사람들에게 투표/질문을 던져 답글 유도 (예: "다들 어떻게 보시나요? [A]다 vs [B]다", "다들 체감은 어떤지 궁금하네 🤔")
...
아래 기사를 바탕으로, 읽자마자 다음 트윗을 보게 만드는 **'2단 스레드(타래)'**를 한국 트위터 트렌드에 맞춰 자연스럽게 작성하세요.
...
2. [2번째 트윗 (2/2)]: 대중이 놓친 구조적 팩트/반전 결론/인사이트 + 자연스러운 생각거리 질문이나 통찰 (끝에 (2/2) 표시)
   - 🚨 **[2번째 트윗 문체/어투 필수 지침]**:
     - 절대 영어 번역투나 어색한 의문문("당신이 ~하는가?", "어떤 데이터베이스를 쓰고 있는가?")을 쓰지 마세요.
     - 한국 트위터 헤비유저들이 쓰는 자연스러운 대화체 질문 또는 깔끔한 통찰로 마무리하세요.
     - 추천 마무리 예시:
       • "현업에서 직접 겪어보신 분들은 체감이 어떤지 궁금하네요"
       • "실제 써보신 분들은 어떻게 느끼셨나요?"
       • "다들 이번 사안은 어떻게 보고 계신가요?"
       • "결국 실사용자 입장에선 이 부분이 가장 큰 변수가 될 듯"
3. 출력 형식은 반드시 아래 JSON 형식으로만 출력하세요:
{
