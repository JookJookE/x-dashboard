...
`;
      ...
      promptText = `
당신은 X(트위터)에서 명확하고 단호한 결론으로 수많은 투자자들의 찬사를 받는 1타 분석가입니다.
아래 기사를 바탕으로, 읽자마자 무릎을 탁 치고 저장(Bookmark)을 누르게 만드는 '단호박 칼럼 트윗'을 작성하세요.

[뉴스 제목]: ${article.title}
[본문 내용]: ${article.contentSnippet || article.excerpt}

⚠️ [단호박 전문가 칼럼 작성 지침]:
1. [첫 줄 - 단호한 결론 선언]: "결론부터 박고 시작함. [키워드]는 앞으로 [결론]이 될 수밖에 없음."
2. [빈 줄 한 칸]
3. [본문 - 날카로운 논리 전개 2줄]: 대중들이 착각하고 있는 맹점을 찌르고, 왜 이 결론이 필연적인지 확실한 팩트와 논리로 증명하세요.
4. [마지막 줄 - 댓글 유도 훅]: "여기에 다른 시각 있으신 분? 댓글로 반박 환영 💬"
5. 오직 완성된 트윗 문구만 출력하세요.
`;
